...
4. Edit the `mimuw-lab2022Z/lab03/aerospike/hosts` file to specify your selected Aerospike servers

5. Run the Aerospike installation playbook:
```bash
cd mimuw-lab2022Z/lab03/aerospike
...
1. Spawn two independent terminals, and run simulatneously a thousand increments on record with id=1:
```bash
...
Hint 1: Use the [optimistic concurrency control](https://en.wikipedia.org/wiki/Optimistic_concurrency_control) technique.
Hint 2: Use the `GenerationPolicy.EXPECT_GEN_EQUAL` generation policy in `writePolicy`, set the `writePolicy.generation` to `record.generation`
Hint 3: React accordingly to the `AerospikeException` with the `resultCode` property equal to: `ResultCode.GENERATION_ERROR`
